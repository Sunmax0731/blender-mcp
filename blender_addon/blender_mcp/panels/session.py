...
class BLENDERMCP_PT_session(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        controls.operator("blendermcp.plan_prompt", text="Plan")
        controls.operator("blendermcp.confirm_prompt_plan", text="Confirm")
        controls.operator("blendermcp.execute_prompt_plan", text="Execute")
        controls.operator("blendermcp.process_next_command", text="取得")
        controls.operator("blendermcp.clear_history", text="履歴消去")
        layout.separator()
        layout.label(text="Plan")
        plan_box = layout.box()
        self._draw_multiline(plan_box, state.prompt_plan_text, fallback="実行計画はまだありません。")
        layout.separator()
        layout.label(text="Preview")
        preview_box = layout.box()
        self._draw_multiline(preview_box, state.prompt_preview_text, fallback="Preview はまだありません。")
        layout.separator()
