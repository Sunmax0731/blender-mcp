...
#### 9.2.1b 画像入力による `character_spec` 補強要件

- 画像入力は自然文 prompt の代替ではなく、`character_spec` 補強入力として扱う
- 初期に受け入れる画像種別は、少なくとも次とする
  - `front`
  - `side`
  - `back`
  - `face_closeup`
...
- 画像入力の優先順位は次の順とする
  ...
  4. `face_closeup`
  5. `expression_*`
- `front` のみでも受理可能とするが、形状比率を補強する場合は `side` を強く推奨する
- 画像入力は、少なくとも次の `character_spec` 項目の補強根拠にならなければならない
  - 顔比率
  - 髪シルエット
  - 体型比率
  - 主要配色
  - 模様位置
  - 目、口、眉の特徴
- prompt と画像が競合する場合の初期ルールは次とする
  - 類型、用途、必要表情は prompt を優先
  - 形状比率、髪シルエット、主要配色、模様位置は画像を優先
  - 矛盾が大きい場合は artifact に conflict を残す
- 画像入力の受け入れ形式は `png` または `jpg` を基本とする
- 背景は単純で、対象キャラと分離しやすいことを推奨する
- 画像入力がある場合、pipeline は `image_reference_manifest` を artifact に残さなければならない
...
- `.vrm` は直接の最優先形式には含めず、承認済み VRM import add-on による `.blend` 前処理変換対象として扱う
...
- `.vrm` を受け取った場合は、流用可能資産の検査の前に `.blend` 変換 artifact を作成し、その結果を入力契約に残さなければならない
